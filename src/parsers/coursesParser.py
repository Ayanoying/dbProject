...
class CoursesParser:
    def __init__(
        self, file_path
    ):  # recoit le chemin vers le fichier CSV & déclenche le chargement
        self.file_path = file_path
        self.data = (
            self._load()
        )  # charge les données en mémoire dès la création de l'objet
    ...
    def _load(self):
        ...
        with open(self.file_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                row["credits"] = int(
                    row["credits"]
                )  # Convertit la colonne crédits de texte en nombre entier
                courses.append(row)
        return courses
