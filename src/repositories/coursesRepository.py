...
class CoursesRepository:
    def save_many(self, courses): # recoit une liste de cours et les insère dans la table 
        conn = get_connection()
        cur = conn.cursor()
        for c in courses:
            cur.execute("""
    ...
    def get_all(self): #retourne tous les cours de la bases trié par code & retourne liste de tuples 
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("""
    ...
    def add_course(self, code_cours, nom, faculte, credits): # insère un seul cours et retourne True si il est inséré & False si le code existe deja 
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO courses (code_cours, nom, faculte, credits)
            ...
            (code_cours, nom, faculte, credits)) #RETURNING code_cours permet de voir si l'insert = réussi 
